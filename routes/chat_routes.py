...
@chat.route('/chat_list', methods=['GET'])
@login_required
def chat_list():
    user_chats = UserChat.query.filter_by(username=current_user.name).order_by(UserChat.last_chat_time.desc()).all()
    chat_list_data = []
    ...
    for user_chat in user_chats:
        chat_list_data.append({
            ...
            'model': user_chat.model,
            'last_chat_time': user_chat.last_chat_time.isoformat()
        })
...
@chat.route('/chat_file', methods=['POST'])
@login_required
def chat_file():
    data = request.get_json()
    # 从 JSON 数据中提取值
